...
2. Admin selects seed candidates or starts an automated collection job.
...
| Refresh queue             | `MatchupRankCoverageQueuePanel`     | `getMatchupRankCoverageQueue`, same file                                                | Filters, sort, limit capped at 20                                                              | `riot_matchup_observations`, `riot_seed_candidates`, rank fields                          | None                                                                                                   | No          | No                                  | Expand the advanced lazy-loaded tool, then select participants missing rank coverage.  |
...
- The Counter Pick admin page no longer renders `RiotIdResolverPanel` or resolver result rows.
- The resolver-specific admin server action and UI-only result types were removed with the panel.
- Shared Riot account lookup utilities remain because `scripts/riot-get-puuid.mjs` still uses `scripts/lib/riot-account-lookup.mjs`.
- The separate encrypted Summoner ID to PUUID path used by automated ladder discovery remains intact.

Admin loading behavior:

- `MatchupRankCoverageQueuePanel` is collapsed by default and does not fetch queue data until expanded.
- Closing and reopening the panel reuses already-loaded queue data.
- The explicit Refresh queue and Apply filters actions still refetch.
- The queue default and maximum result limit is 20 in the UI, server action clamp, and queue library.
- Riot Seed Candidate rank-group expansion is controlled independently from candidate refreshes. Refresh, pagination, and selection preserve the user's expanded or collapsed group state.
