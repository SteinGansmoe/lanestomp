import { matchupDraftSectionKeys, type MatchupDraftSectionKey } from "./matchup-draft-prompt";
...
const fallbackContaminationPatterns: readonly MatchupFallbackContaminationPattern[] = [
  ...
  },
  {
    ...
    pattern: /\bstrongest invade, river, or objective timing makes blind fog checks dangerous\b/i,
  },
  {
  ...
  },
  {
    ...
    pattern: /\bif you have a numbers disadvantage, trade the tempo window into top-side camps\b/i,
  },
  {
...
export function scanMatchupDraftForFallbackContent(
  ...
  const affectedSections = Array.from(new Set(matches.map((match) => match.sectionKey)));
