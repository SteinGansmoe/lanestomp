...
export const pykeCombatProfile = {
  ...
  lanePlan: {
    avoids: ["Missing hook in lane.", "Roaming on bad waves.", "Trying to peel like a tank."],
    idealLaneState:
      "A volatile lane where Pyke threatens brush hooks, creates roam timers after wave crash, and looks for low-health execute chains.",
    wants: ["Fog control.", "Burst ADC follow-up.", "Enemy bot lane below execute threshold."],
  },
  laneIdentity: {
    earlyGameAgency: "high",
    lanePressure: "high",
    preferredGameState: ["Snowball lanes.", "Roam-heavy maps.", "Skirmishes with execute resets."],
    scalingPriority: "low",
    winLaneBy: ["Landing hook.", "Creating numbers advantages.", "Executing low-health targets."],
  },
  majorPowerSpikes: [
    "Level 2 hook plus stun threat.",
    "Level 3 stealth roam threat.",
    "Level 6 execute reset.",
  ],
  matchupPreferences: {
    strongInto: [
      "Immobile enchanters.",
  ...
  primaryTradingPattern:
    "Hide hook threat from brush or (W), pull targets into ADC burst, then use (E) to stun the escape path.",
  punishProfile: {
    canPunish: [
  ...
  trading: {
    badTradeConditions: ["(Q) misses.", "(E) is used forward.", "His roam leaves ADC exposed."],
    goodTradeConditions: [
      "Fog is controlled.",
      "Enemy flash is down.",
